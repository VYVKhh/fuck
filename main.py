...
bot_token = '8107614050:AAFBFhW_c5a2miZBG8UJYl-xPsslPunhckI'  # تم تضمين التوكين الخاص بك

bot = Client("my_session", api_id=api_id, api_hash=api_hash, bot_token=bot_token)
...
def send_start(client: pyrogram.client.Client, message: pyrogram.types.messages_and_media.message.Message):
    ...
    if ss is None:
        bot.send_message(message.chat.id, "👋 مرحبًا! من فضلك، أرسل جلسة pyrogram الخاصة بك.")
    else:
        acc = Client("myacc", api_id=api_id, api_hash=api_hash, session_string=ss)
        acc.start()
        bot.send_message(message.chat.id, f"__👋 مرحبا **{message.from_user.mention}**, انا بوت حفظ المنشورات من القنوات المقيدة. أرسل لي رابط المنشور.",
        reply_markup=InlineKeyboardMarkup([[ InlineKeyboardButton("السورس", url="https://t.me/V_0_E")]]), reply_to_message_id=message.id)
...
def save(client: pyrogram.client.Client, message: pyrogram.types.messages_and_media.message.Message):
    ...
    if ss is None:
        ...
        acc = Client("myacc", api_id=api_id, api_hash=api_hash, session_string=ss)
        acc.start()
        bot.send_message(message.chat.id, "تم تعيين سلسلة الجلسة بنجاح! الآن يمكنك إرسال رابط المنشور.")
    else:
...
def handle_private(message, chatid, msgid):
    ...
    if "Document" in str(msg):
        ...
        bot.send_voice(message.chat.id, file, caption=msg.caption, reply_to_message_id=message.id, progress=progress, progress_args=[message, "up"])
